...
try{
  ...
  $sql = " SELECT m.MEMBER_IMAGE, f.GROUP_NAME, r.RES_NAME, DATE(f.MEAL_TIME) 'DATE',Date_format((f.MEAL_TIME),'%H:%i') 'TIME', f.GROUP_NO, fgp.MEMBER_NO
  FROM `food_group` f JOIN `food_group_people` fgp ON (f.GROUP_NO = fgp.GROUP_NO)
          JOIN `restaurant_management` r ON (r.RES_NO = f.RES_NO)
                  JOIN `member_management` m ON (m.MEMBER_NO = f.MEMBER)
  WHERE  fgp.MEMBER_NO IN (SELECT fdp1.MEMBER_NO
                        FROM `food_group_people`fdp1 JOIN `member_management` m1 ON(fdp1.MEMBER_NO = m1.MEMBER_NO)
                        WHERE  m1.MEMBER_ID =:MEMBER_ID)
  AND (fgp.MEMBER_STATUS = 0  
  OR fgp.MEMBER_STATUS = 2) ";
